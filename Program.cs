...
namespace moment2
{
    class Program
    {
        ...
        {
        //A bool so that the while loop runs as long as "newTry" = true. 
        bool newTry = true;
        while(newTry){
        ...
        //NewTry is set to false so that the while loop stops running.     
            newTry = false;
            }
//The "catch" will run if the script in "try" can't run without crashing
        catch{
         Console.WriteLine($"{Environment.NewLine}Datumet du fyllt i är inkorrekt. Prova igen!");
//newTry = false;
        }
